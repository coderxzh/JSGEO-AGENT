...
export function Drafts() {
  return (
    <div className="p-xl max-w-7xl mx-auto animate-in fade-in duration-500">
      
      {/* Header */}
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-[32px] font-bold text-primary tracking-tight">稿件创作与分发管理</h2>
          <p className="text-[14px] text-on-surface-variant mt-2 max-w-2xl">创作符合大模型算法偏好的科普干货、深度测评及排行推荐文章。支持断点恢复人工校准后一键投递分发。</p>
        </div>
        <button className="bg-primary text-on-primary text-[11px] font-bold uppercase tracking-wider px-6 py-3 hover:bg-inverse-surface transition-colors duration-200 flex items-center gap-2 rounded-md shadow-sm">
      ...
      <div className="flex justify-between items-center mb-6 pb-6 border-b border-outline-variant/30">
        ...
        {/* Segmented Control */}
        <div className="flex p-1 bg-surface-container-low border border-outline-variant/40 rounded-lg shadow-sm">
          <button className="px-5 py-2 text-[11px] font-bold uppercase tracking-wider bg-surface-container-lowest text-primary shadow-sm rounded-md border border-on-surface/5 transition-all">全部</button>
        ...
        {/* Secondary Filters */}
        <button className="flex items-center gap-2 px-5 py-2.5 border border-outline-variant/50 text-primary text-[11px] font-bold uppercase tracking-wider hover:bg-surface-container-low transition-colors rounded-md shadow-sm">
      ...
      {/* Data Table */}
      <div className="bg-surface-container-lowest border border-outline-variant/40 rounded-2xl shadow-sm overflow-hidden backdrop-blur-md bg-opacity-90">
...
function DraftRow({ title, id, status, color, score, scoreOpacity, date, isLow, isLast }: any) {
  return (
    <tr className={cn(
      "hover:bg-surface-container-low transition-colors duration-200 group",
      ...
      <td className="px-6 py-5">
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-outline-variant/50 bg-surface font-mono text-[11px] font-medium shadow-sm">
